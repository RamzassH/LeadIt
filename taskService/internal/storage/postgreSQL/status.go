...
func (s *StatusStorage) Save(ctx context.Context, payload models.CreateStatusDTO) (statusId int64, err error) {
	...
	query := `INSERT INTO statuses (name, order, progress_bar_id, notify_roles_ids, project_id) VALUES ($1. $2, $3, $4, $5) RETURNING id;`

	err = s.db.QueryRowContext(ctx, query, payload.Name, payload.Order, payload.ProgressBarID, payload.NotifyRolesIDS, payload.ProjectID).Scan(&statusId)
	if err != nil {
...
func (s *StatusStorage) GetManyByProjectId(ctx context.Context, projectId int64) (statusList []*models.StatusDTO, err error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT * FROM statuss WHERE project_id = $1`, projectId)
	if err != nil {
		return nil, fmt.Errorf("%s: %w", op, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(item.ID, item.ProjectID, item.Name, item.NotifyRolesIDS, item.Order, item.ProgressBarID); err != nil {
			return nil, fmt.Errorf("%s: %w", op, err)
		}
...
func (s *StatusStorage) Update(ctx context.Context, payload models.UpdateStatusDTO) (*models.StatusDTO, error) {
	...
	query := `UPDATE statuss SET 
                   mame = COALESCE($1, name),
